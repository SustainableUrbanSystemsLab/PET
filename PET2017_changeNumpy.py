...
def Syst(T, Ta, Tmrt, HR, v, age, sex, ht, mbody, pos, M, icl, mode):
    ...
    alpha = vasoC(T[0], T[1])[1]
    tbody = alpha * T[1] + (1 - alpha) * T[0]
    ...
    Pvsk = 6.105 * \
        math.exp((17.27 * (T[1]+273.15) - 4717.03) /
                 (237.7 + T[1]))  # [hPa]
    ...
    rbare = Aeffr*(1.0 - facl) * emsk * sigm * ((Tmrt + 273.15)
                                                ** (4.0) - (T[1] + 273.15)**(4.0))/Adu
    # For dressed area:
    rclo = feff * Aclo * emcl * sigm * \
        ((Tmrt + 273.15)**(4.0) - (T[2] + 273.15)**(4.0))/Adu
    rsum = rclo+rbare

    # Convection losses #
    cbare = hc * (Ta - T[1]) * Adu * (1.0 - facl)/Adu  # [w/m^2]
    cclo = hc * (Ta - T[2]) * Aclo/Adu  # [W/m^2]
    csum = cclo+cbare

    # Balance equations of the 3-nodes model
    enbal_vec[0] = h + ere - (vasoC(T[0], T[1])[0] / 3600*cb+5.28)*(T[0]-T[1])  # Core balance [W/m^2]
    enbal_vec[1] = rbare + cbare + evap + (vasoC(T[0], T[1])[0]/3600*cb+5.28)*(T[0]-T[1]) - htcl*(T[1]-T[2])  # Skin balance [W/m^2]
    enbal_vec[2] = cclo + rclo + htcl *  (T[1]-T[2])  # Clothes balance [W/m^2]
    ...
    if mode:
        ...
        return [enbal_vec[0], enbal_vec[1], enbal_vec[2]]
    else:
...
Tmrt = 20  # Mean radiant temperature in [oC]
...
print("Tstable1", Tstable)

print("SystFALSE", Syst(Tstable, Ta, Tmrt, HR,
                   v, age, sex, ht, mbody, pos, M, icl, False))

print("Syst", Syst(Tstable, Ta, Tmrt, HR,
                   v, age, sex, ht, mbody, pos, M, icl, True))
